  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        ...
        setQuestions(shuffled.slice(0, 100)); // Ambil 100 soal random
        setLoading(false);
      } catch (error) {
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 pb-4 border-b dark:border-gray-700 gap-4">
            ...
            <div className="flex flex-wrap items-center gap-1 sm:gap-2 w-full sm:w-auto justify-start sm:justify-end">
              <button
                ...
                className={`rounded px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:text-sm font-medium transition-colors ${
                      ...
                      <div key={diff} className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${
                            diffPercentage >= 80 ? 'bg-green-100 text-green-800' :
              ...
              {questions.map((_, index) => {
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-8 sm:w-10 sm:h-10 rounded-md font-medium text-xs sm:text-sm transition-all transform hover:scale-105 relative ${
                      isCurrent
              ...
              {Object.entries(q.options || {}).map(([key, value]) => {
                ...
                return (
                  <button
                    ...
                    disabled={showResult}
                    className={`flex w-full items-start p-3 sm:p-4 rounded-lg border cursor-pointer transition-colors text-left ${
                      answer === key && !showResult
                        ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20'
                        : bgClass
                    } ${textClass} ${
                      !showResult ? "hover:bg-gray-50 dark:hover:bg-gray-800" : "cursor-default"
                    }`}
                  >
